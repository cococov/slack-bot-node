import { getUserInGitlab, getUsersByTeamInFirebase } from '../../util/util.js'
...
export const remove = async ({ bot, channel, userId, teamName, args: { 'user-add':username } }) => {
  const userGitlab = await getUserInGitlab(username);
  if(!(typeof userGitlab === 'object')) { return bot.postMessage(channel, `ups! ${username} no existe en gitlab`);}

  const usersFirebase = await getUsersByTeamInFirebase(teamName);
  if(usernames(usersFirebase).includes(username)) { return bot.postMessage(channel, `ups! ${username} ya existe en equipo ${teamName}`);}
...
const usernames = usersFirebase => usersFirebase.map(user => user.username)
